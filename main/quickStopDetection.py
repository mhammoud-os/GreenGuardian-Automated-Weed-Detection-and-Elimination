...
def moveforward(amount):
    if round(amount/22*40) <2:
        return
    sendDataOverSerialPort("m")
    sendNum(round(amount/22*40))
...
while(True):
    ...
    for i in range(len(prediction['score'])):
        if ((prediction['score'][i] > min_conf_threshold) and (prediction['score'][i] <= 1.0)):
            ...
            for i in range(len(prediction['score'])):
                if ((prediction['score'][i] > min_conf_threshold) and (prediction['score'][i] <= 1.0)):
                    move_amount =(prediction['boxes'][i][0] + prediction['boxes'][i][2])/2 
            ...
            for i in range(len(weedsPos)):
                ...
                moveforward(weedsPos[i][1]*13.5)
                #moveforward(weedsPos[i][1]*5)
                print(weedsPos[i][1])
                print(weedsPos[i][0])
